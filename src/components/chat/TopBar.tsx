import { Phone, ArrowRightLeft, BellOff, X } from "lucide-react";
...
const TopBar = ({ customerName, status, connectionStatus }: TopBarProps) => {
  const statusLabel =
    status === "typing"
      ? "Typing…"
      : status === "online"
    ...
    <div className="h-16 border-b bg-card flex items-center justify-between px-6 shrink-0">
      <div className="flex items-center gap-3">
        <div className="relative">
          ...
          <span
            className={`absolute bottom-0 right-0 w-3 h-3 rounded-full border-2 border-card ${statusColor}`}
          />
        </div>
        <div>
